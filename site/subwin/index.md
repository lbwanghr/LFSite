...
Download a Windows 11 ARM ISO from Microsoft, point SubWin to it, and let the automated installation run. Once finished, SubWin lives in your menu bar and waits there in seconds-ready state — connect to `localhost:3389` from any RDP client whenever you need Windows. Microsoft's free [Windows App](https://apps.apple.com/app/windows-app/id1295203466) works great, on macOS, iPad, iPhone, or another Mac.
...
Yes. SubWin runs Windows 11 ARM, but the Windows license itself is yours to obtain (Home, Pro, or Education edition). Download the ARM ISO directly from Microsoft.
...
Yes. SubWin listens on `0.0.0.0:3389`, so any device on your local network can connect with an RDP client. Combined with tools like Tailscale or Cloudflare Tunnel, you can also reach it from outside your home network.
